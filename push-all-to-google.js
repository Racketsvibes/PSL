...
async function run() {
  ...
  try {
    ...
    console.log("Got access token, starting to submit URLs...");
    ...
    for (const url of urls) {
      try {
        ...
        if (res.ok) {
          ...
          console.log(`❌ Failed: ${url} - ${JSON.stringify(result)}`);
          failCount++;
        }
    ...
    console.log(`\nFinished! Successfully indexed ${successCount}/${urls.length} URLs.`);
    if (failCount > 0) console.log(`${failCount} URLs failed.`);
